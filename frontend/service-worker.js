...
// already-installed worker. The new Function, backend/netlify/functions/discover-ai.js, is never a static
...
// proxy Function (backend/netlify/functions/anilist.js) is never a static browser asset (Function source
...
// are never reachable from the browser at all in this design (only backend/netlify/functions/anilist.js
...
// Function (backend/netlify/functions/weather.js, never part of PRECACHE — Function source is never a
...
// backend/netlify/functions/lib/tools.js/qwen.js, which aren't part of PRECACHE) — plus a new calendar
...
// keys these render with. backend/netlify/functions/assistant.js's system prompt also changed (scope
...
// assistant.evidence_* keys these render with. backend/netlify/functions/**/*.js also changed
...
// from backend/netlify/functions/assistant.js) — nothing here can ever intercept that call — this SW's
...
// assistant.* i18n keys). backend/netlify/functions/**/*.js also changed but was never part of PRECACHE
...
// already-active v23 worker's stale copy. backend/netlify/functions/assistant.js and its lib/ modules
...
// v48 (Calendar/Google Calendar cleanup): return the Calendar shell to its protected baseline,
...
const BYPASS_HOSTS = [
  ...
  // scope (the browser never calls it directly — only backend/netlify/functions/assistant.js does,
...
if (hasUsableFirebaseConfig(fcmConfig)) {
  ...
  try {
    ...
    // The server (backend/netlify/functions/anime-airing-check.js) always sends a DATA-only payload
