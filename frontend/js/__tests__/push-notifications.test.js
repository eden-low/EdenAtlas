...
// frontend/js/__tests__/discover-security.test.js and frontend/js/__tests__/xss-security.test.js already
...
//
// Run with: node frontend/js/__tests__/push-notifications.test.js (or `npm run test:frontend`).
...
const SOURCE = fs.readFileSync(path.join(ROOT, "frontend", "js", "push-notifications.js"), "utf8");
...
(async () => {
  ...
  // client sees as PERMISSION_DENIED, not NOT_FOUND). firebase/firestore/__tests__/discover-rules.test.js
  ...
  // firebase/firestore/__tests__/discover-rules.test.js; this proves what the CLIENT sends, not what the
  // server would accept.
