...
// "existing test style" constraints (see frontend/js/__tests__/date-utils.test.js/reflection.test.js).
//
// Run with: node frontend/js/__tests__/home-recent-memories.test.js
...
const HOME_HTML = fs.readFileSync(path.join(ROOT, "frontend", "pages", "home.html"), "utf8");
...
await test("service-worker.js CACHE has been bumped to at least eden-shell-v31 (this fix's own bump, home.html is precached)", () => {
  const sw = fs.readFileSync(path.join(ROOT, "frontend", "service-worker.js"), "utf8");
...
await test("html::before (the decorative EdenAtlas pulse mark) is non-interactive: pointer-events:none, painted behind all content", () => {
  const styles = fs.readFileSync(path.join(ROOT, "frontend", "styles", "styles.css"), "utf8");
...
await test("the splash screen's logo image is marked decorative (empty alt) and its overlay stops intercepting clicks once it starts fading out", () => {
  const splash = fs.readFileSync(path.join(ROOT, "frontend", "js", "splash.js"), "utf8");
