...
// frontend/js/__tests__/home-recent-memories.test.js's extractFunctionSource(), so a future edit to any
...
//
// Run with: node frontend/js/__tests__/xss-security.test.js
...
const FRONTEND_JS = path.join("frontend", "js");
const JOURNAL_SRC = readSrc(path.join(FRONTEND_JS, "journal.js"));
const CAREER_SRC = readSrc(path.join(FRONTEND_JS, "career.js"));
...
await test("esc() is byte-identical (drift check) across every file fixed in the stored-XSS pass -- a future edit to one copy without the others would silently reopen the gap in the un-updated files", () => {
  ...
  files.forEach((f) => {
    const src = readSrc(path.join(FRONTEND_JS, f));
