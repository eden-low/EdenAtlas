// backend/netlify/functions/lib/build-context.js — reads the build-time Netlify CONTEXT/BRANCH snapshot
...
// here too so backend/netlify/functions/lib/firebase-admin.js's assertProjectMatchesBuildContext() can
...
// esbuild bundles backend/netlify/functions/anilist.js (and friends) into ONE output file that keeps the
// ENTRY file's own original relative path inside the deployed archive
// (backend/netlify/functions/anilist.js) — but every module IT required, including this one, gets INLINED
...
// bundle, `__dirname` resolved to `backend/netlify/functions/`, one directory ABOVE this file's real
// original location (`backend/netlify/functions/lib/`). `included_files` (a prior fix attempt) DID
...
// (backend/netlify/functions/lib/build-context.generated.json), which no longer matches the bundle's
...
// unbundled `node backend/netlify/functions/...` invocation (Node's own require() resolves a JSON module
