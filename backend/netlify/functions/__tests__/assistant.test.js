...
//
// These tests exercise backend/netlify/functions/assistant.js's `createHandler(deps)` factory directly —
...
async function run() {
  ...
  await test("repository guard: zero legacy firebase-admin namespace calls remain in production Function code (comments excluded)", async () => {
    ...
    // durable, low-maintenance regression check: it scans every .js file under backend/netlify/functions
    ...
    const functionsDir = path.join(root, "backend", "netlify", "functions");
  ...
  await test("static check: assistant.js routes every scope checkbox change and setScopeChecked() through applyScopeChange(), never a bare saveScopes()", async () => {
    const root = path.resolve(__dirname, "..", "..", "..", "..");
    const src = fs.readFileSync(path.join(root, "frontend", "js", "assistant.js"), "utf8");
  ...
  await test("static check: gallery.js/journal.js/timeline.js each resolve their deep-link query param only against already-fetched, uid/rules-scoped cached data", async () => {
    ...
    for (const { file, param, cacheVar } of cases) {
      const src = fs.readFileSync(path.join(root, "frontend", "js", file), "utf8");
  ...
  await test("EN/ZH locale files have identical key sets (including the new assistant.* namespace)", async () => {
    const root = path.resolve(__dirname, "..", "..", "..", "..");
    const en = JSON.parse(fs.readFileSync(path.join(root, "frontend", "locales", "en.json"), "utf8"));
    const zh = JSON.parse(fs.readFileSync(path.join(root, "frontend", "locales", "zh-CN.json"), "utf8"));
  ...
  await test("scope-change and no-scope notices are wired to real translated text in both locales, and their DOM elements exist in assistant.html", async () => {
    ...
    const html = fs.readFileSync(path.join(root, "frontend", "pages", "assistant.html"), "utf8");
  ...
  await test("service-worker.js: /.netlify/functions/* requests are never written to Cache Storage", async () => {
    const root = path.resolve(__dirname, "..", "..", "..", "..");
    const src = fs.readFileSync(path.join(root, "frontend", "service-worker.js"), "utf8");
  ...
  await test("service-worker.js CACHE version is at least eden-shell-v28 (this suite predates later cache bumps — see weather.test.js for the current version) and includes assistant.html/js/assistant.js in PRECACHE", async () => {
    const root = path.resolve(__dirname, "..", "..", "..", "..");
    const src = fs.readFileSync(path.join(root, "frontend", "service-worker.js"), "utf8");
    ...
    assert.ok(/"js\/assistant\.js"/.test(src));
    assert.ok(/"js\/gallery\.js"/.test(src));
    assert.ok(/"js\/journal\.js"/.test(src));
    assert.ok(/"js\/timeline\.js"/.test(src));
