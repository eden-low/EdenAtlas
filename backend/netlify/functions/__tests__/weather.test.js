...
// real provider key. Run with: node backend/netlify/functions/__tests__/weather.test.js (or
...
async function run() {
  ...
  // this file ever needing to know either value. backend/netlify/functions/weather.js is the one
  ...
  await test("no browser file calls OpenWeatherMap directly, embeds appid=, or contains a key-shaped literal; the Function reads its key only from the environment", async () => {
    ...
    const fnSrc = fs.readFileSync(path.join(root, "backend", "netlify", "functions", "weather.js"), "utf8");
