...
async function run() {
  ...
  await test("secrets are absent from errors and from browser-published source", async () => {
    ...
    assert.strictEqual(response.statusCode, 500);
    assert.ok(!response.body.includes(SECRET));
    const buildSitePath = path.resolve(__dirname, "..", "..", "..", "..", "scripts", "build-site.js");
    const { SOURCE_FILE_MAPPINGS, SOURCE_DIR_MAPPINGS } = require(buildSitePath);
    const publicTargets = [...SOURCE_FILE_MAPPINGS, ...SOURCE_DIR_MAPPINGS].map(({ target }) => target);
    assert.ok(publicTargets.every((target) => !target.startsWith("netlify/") && !target.startsWith("backend/")));
  });
